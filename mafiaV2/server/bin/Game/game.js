...
// assign roles
function assignRoles (users, mafiaCount, doctorCount, copCount) {
    var clientIDs = [];
...
module.exports =  {
   start : function(socket, lobbyCode) {
      ...
      Connection.db.collection("game_" + lobbyCode).findOne(query).then((data) => {
        ...
        var roles = assignRoles(users, gameObject.mafiaCount, gameObject.doctorCount, gameObject.copCount);
        console.log(roles);
